...
describe('tags', () => {
  it('posts content with top_n and concepts and returns the typed group dict', async () => {
    const groups = {
      entities: ['Acme Corp'],
    ...
    const result = await client.tags('some text', 5, 'concepts, entities, topics');
    expect(result).toEqual(groups);
    const [url, options] = mock.mock.calls[0];
    expect(url).toContain('top_n=5');
    expect(url).toContain('concepts=concepts%2C+entities%2C+topics');
    expect(JSON.parse(options.body)).toEqual({ content: 'some text' });
  });

  it('throws on a failed response', async () => {
    mock_fetch({}, false);
    await expect(client.tags('text', 5, 'concepts')).rejects.toThrow(/tags/);
  });
});
...
describe('score', () => {
  it('posts query and candidates and returns similarity scores', async () => {
    const mock = mock_fetch({ scores: [0.9, 0.2] });
    const result = await client.score('query text', ['alpha', 'beta']);
    expect(result).toEqual([0.9, 0.2]);
    const call_body = JSON.parse(mock.mock.calls[0][1].body);
    expect(call_body.candidates).toEqual(['alpha', 'beta']);
  });
});
