#!/usr/bin/env node
/**
 * graphly CLI: generate a treemap tree data file from a text document via
 * Taggly, or launch the sample viewer for an existing data file. All settings
 * come from the YAML config file (./config.yaml or --config <path>).
 */
...
const count_nodes = (node: TreeNode): number =>
  1 + (node.children ?? []).reduce((sum, child) => sum + count_nodes(child), 0);

const program = new Command('graphly')
  .description('Treemap page-index data generator built on Taggly')
...
program.argument('<input>', 'text document to graph')
  .action(async (input) => {
    ...
    const tree = await generate_graph(text, input, config, client);
    writeFileSync(config.output, JSON.stringify(tree, null, 2));
    console.log(`wrote ${count_nodes(tree.root)} nodes to ${config.output}`);
  });
